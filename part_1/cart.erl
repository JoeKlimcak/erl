...
handle_call({start_link, UserName}, _From, State) ->
  ...
  {ok, Pid} = cart_fsm:start_link(UserName),
...
handle_call({checkout, ReferenceId}, _From, State) ->
  UserName = db:get_username(ReferenceId),
  {reply, cart_fsm:checkout(UserName), State};
...
handle_call({delivered, ReferenceId}, _From, State) ->
  case db:check_user(ReferenceId) of
    false ->
      {reply, {error, unknown}, State};
    true ->
      UserName = db:get_username(ReferenceId),
...
handle_info({'EXIT', Pid, _}, State = #server_state{}) ->
  ReferenceId = db:get_ref_id_from_pid(Pid),
